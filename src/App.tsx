...
function AuthForm({ mode, onAuth, onSwitch, currentTheme, onThemeChange }: {
  ...
  return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: C.bg }}>
    <div className="card anim-fadeUp" style={{ width: "100%", maxWidth: 440, padding: 36 }}>
      ...
      <div style={{ fontSize: 14, color: C.muted, marginBottom: 24 }}>{mode === "login" ? t.loginSub : t.signupSub}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {mode === "signup" && <input className="input-field" placeholder={t.namePlaceholder} value={name} onChange={e => setName(e.target.value)} />}
        <input className="input-field" type="email" placeholder={t.emailPlaceholder} value={email} onChange={e => setEmail(e.target.value)} />
        <input className="input-field" type="password" placeholder={t.passwordPlaceholder} value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
        {mode === "signup" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {PLANS.map(p => (
                <button key={p.id} onClick={() => setPlan(p.id as Profile["plan"])}
                  ...
                  {p.name}<br /><span className="mono" style={{ fontSize: 11 }}>{p.price === 0 ? t.free : `€${p.price}/mo`}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign: "center", fontSize: 13, color: C.muted }}>
          ...
          <span style={{ color: C.accent, cursor: "pointer" }} onClick={onSwitch}>{mode === "login" ? t.signupFree : t.loginLink}</span>
        </div>
      </div>
    </div>
  </div>;
