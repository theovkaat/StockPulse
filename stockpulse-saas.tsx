    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{ padding: "0 40px 60px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
          {[
            ...
            { icon: "🔔", title: "Smart Alerts", desc: "Set price alerts and get an instant email when your target price is reached." },
            { icon: "🤖", title: "AI Market Analysis", desc: "Weekly AI analysis of your portfolio with opportunities and risks per position." },
            { icon: "🌍", title: "Global Markets", desc: "AEX, Nasdaq, NYSE, LSE — everything on one dashboard. Multiple currencies." },
          ].map((f, i) => (
            <div key={i} className="card card-hover" style={{ padding: "24px", transition: "all 0.2s" }}>
      ...
      <div style={{ padding: "0 40px 80px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
          {PLANS.map((plan, i) => (
            <div key={plan.id} className="card" style={{ padding: "28px 24px", border: `1px solid ${plan.id === "pro" ? C.accent + "66" : C.border}`, position: "relative", transition: "all 0.2s" }}>
              {plan.id === "pro" && (
                <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: C.accent, color: "white", fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 100, letterSpacing: 0.5 }}>MOST POPULAR</div>
              )}
